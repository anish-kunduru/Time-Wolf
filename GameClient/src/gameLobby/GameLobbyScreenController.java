...
public class GameLobbyScreenController implements ControlledScreen {
	...
	public void initialize() {
		...
		try {
			System.out.println(gameManagement.listJoinableGames().size());
		} catch (RemoteException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		...
		try {
			...
			if (games != null) {
				...
				for (GameInfo currentGame : games) {
					...
					currentEntry.name.set(currentGame.getName());// Set game
																	// name.
					...
					// permits:
					currentEntry.chat.set(true); // Chat will be enabled for all
													// games for now.
					currentEntry.privateLobby.set(false); // All lobbies will be
															// public for now.
			...
			// Note that this won't work at initial launch because of the way
			// the controller is created... This would be fixed with my revised
			// controller.
			else
