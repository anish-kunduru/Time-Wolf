	...
	public void initialize() {
		...
		try {
			users = MainModel.getModel().currentLoginData()
					.getLogInConnection().getLeaderboard();
			int count = 1;
			...
			for (int i = 0; i < users.size(); i++) {
				...
				if (!user.isBanned()) {
					...
					currentRow.gamesPlayed
							.set(user.Statistics.getGamesPlayed());
					currentRow.gamesWon.set(user.Statistics.getGamesWon());
					currentRow.ratio.set(df.format(user.Statistics
							.getWinLossRatio()));
					currentRow.totalPoints.set((int) user.Statistics
							.getTotalPoints());
					currentRow.avgPoints.set((int) user.Statistics
							.getAveragePoints());
